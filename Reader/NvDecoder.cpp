...
void videoDecPostProcessFrame(CUdeviceptr decodedFrame, size_t pitch, CUdeviceptr outFrame, size_t outPitch, int width, int height, cudaStream_t stream);
...
NvDecoder::NvDecoder(bool bUseDeviceFrame, cudaVideoCodec eCodec, bool bLowLatency, 
    ...
    m_eCodec(eCodec), m_bDeviceFramePitched(bDeviceFramePitched),
    m_nMaxWidth (maxWidth), m_nMaxHeight(maxHeight), m_bForce_zero_latency(force_zero_latency)
{
    ...
    NVDEC_API_CALL(cuCtxGetCurrent(&m_cuContext));
